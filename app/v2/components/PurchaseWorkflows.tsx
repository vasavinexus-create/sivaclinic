...
export function PurchaseWorkflow({ profile, notify }: { profile: Profile; notify: (message: string) => void }) {
  ...
  const addItem = (form: HTMLFormElement) => {
    ...
    setItems((rows) => [...rows, { product_id: productId, product_name: product.name, product_code: product.product_id, batch_number: data.get("batch_number"), expiry_date: data.get("expiry_date"), quantity: qty, free_quantity: free, units_per_purchase_unit: units, stock_qty: (qty + free) * units, purchase_rate: rate, purchase_rate_per_unit: rate / units, selling_rate: Number(data.get("selling_rate") || 0), mrp: Number(data.get("mrp") || 0), gst_percent: gst, discount, line_total: lineTotal }]);
    setProductId("");
    setProduct(null);
  };
